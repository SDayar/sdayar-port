...
export default function Temp() {
  ...
  // Ref pour la facture PDF et le canvas de fond
  ...
  // --- ARRIÈRE-PLAN DYNAMIQUE NON-BLOQUANT ---
  useEffect(() => {
  ...
  // --- GENERATION DU PDF OPTIMISEE POUR MOBILE & ORDI ---
  const downloadPDF = () => {
    const element = pdfRef.current
    if (!element) return

    // Ajustement temporaire pour forcer le bon rendu sur petit écran
  ...
  const customStyles = `
    ...
    .temp-section-header { text-align: center; margin-bottom: 30px; }
    ...
    .temp-grid-2 { display: grid; grid-template-columns: repeat(auto-fit, minmax(280px, 1fr)); gap: 16px; }
    .temp-grid-3 { display: grid; grid-template-columns: repeat(auto-fit, minmax(240px, 1fr)); gap: 12px; }
    ...
    .temp-card-item { background-color: #ffffff; border-radius: 16px; padding: 16px; border: 1px solid #e2e8f0; cursor: pointer; transition: all 0.2s ease; position: relative; }
    ...
    .temp-pdf-container { background-color: #ffffff; border-radius: 20px; padding: 24px; border: 1.5px solid #fecdd3; min-width: 280px; max-width: 700px; margin: 0 auto; box-shadow: 0 10px 30px rgba(0,0,0,0.02); box-sizing: border-box; }
        ...
        <div className="temp-pdf-wrapper">
          <div ref={pdfRef} className="temp-pdf-container">
            ...
            <div style={{ marginTop: '20px', borderTop: '2px dashed #ffe4e6', paddingTop: '16px', textAlign: 'right' }}>
              ...
              <strong style={{ fontSize: '1.1rem', color: '#e11d48', marginLeft: '6px' }}>0,00 € + Tout mon Amour</strong>
            </div>
          </div>
        </div>

        <button className="temp-btn-pink" onClick={downloadPDF}>
